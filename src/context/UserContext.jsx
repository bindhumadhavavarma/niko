...
export const AxiosPost = async (apiname, body) => {
    if (window.localStorage.getItem("role") == "ADM_MASTER") {
        const { data } = await Axios.post(apiname, { ...body, selected_movie_id: window.localStorage.getItem("movie") })
        return data;
    } else {
...
export const AxiosGet = async (apiname) => {
    if (window.localStorage.getItem("role") == "ADM_MASTER") {
        const { data } = await Axios.post(apiname, { selected_movie_id: window.localStorage.getItem("movie") })
        return data;
    } else {
...
export const UserContextProvider = ({ children }) => {
    ...
    return (
        <UserContext.Provider value={{ loginUser, wait, user: theUser, loggedInCheck, logout, movieSelectedCheck, movieSelected }}>
            {children}
